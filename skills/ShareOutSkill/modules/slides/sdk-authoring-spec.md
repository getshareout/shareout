...
Each maps to a pattern already documented in [design/](./design/README.md), so output is on-brand by construction. Slots accept either plain text or nested helper HTML.
...
Themes live in the SDK (seeded from [design/](./design/README.md) templates) and emit CSS-var-based output, cascading from the presentation's existing `defaultColors`/`defaultFonts`. No DB change — `withTheme` just parameterizes the HTML generator. (The unused `template` column can later persist the chosen theme name.)
